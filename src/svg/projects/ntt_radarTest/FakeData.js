...
class FakeData{
    ...
    _createData(numItems){
        ...
        const INDUSTRIES_DATA = [
            { id: 'ind_1', name: 'Automotion', color: '#2ecc71' }, 
        ...
        const FOUNDRIES_DATA = [
            { id: 'fnd_1', name: 'Supply Chain', color: '#00d2d3' }, 
        ...
        // Función para obtener un sub-array aleatorio de IDs
        ...
        const talks = Array.from({ length: numItems }, (_, index) => {
            ...
            // Lógica de distribución 20/80
            const distributionRoll = Math.random(); 
            let talkIndustries = [];
            let talkFoundries = [];

            if (distributionRoll < 0.1) {
                // CASO 20%: Tiene AMBOS (Industries Y Foundries)
                talkIndustries = getRandomIds(INDUSTRIES_DATA, 2);
                talkFoundries = getRandomIds(FOUNDRIES_DATA, 2);
            } else {
                // CASO 80%: Tiene UNO U OTRO (Industries O Foundries)
                // Lanzamos una moneda (50/50) para decidir cuál de los dos tiene
                if (Math.random() < 0.5) {
                    talkIndustries = getRandomIds(INDUSTRIES_DATA, 2);
                    // talkFoundries se mantiene vacío []
                } else {
                    talkFoundries = getRandomIds(FOUNDRIES_DATA, 2);
                    // talkIndustries se mantiene vacío []
                }
            }
            ...
            return {
                ...
                industries: talkIndustries,
                foundries: talkFoundries,  
                state: getRandomInt(0, 2)
            };
        });
        ...
        console.log("(FakeData) Database Generated. Sample item:", this.database.talks[0]);
    }

    _createData_old(numItems){
        // --- 1. Definición de Datos Maestros ---
        const INDUSTRIES_DATA = [
